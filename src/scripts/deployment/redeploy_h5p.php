...
if ($h5pid !== false) {
    ...
    if ($h5p) {
        ...
        // Zeige die verknüpften Libraries
        $libs = $DB->get_records_sql("
            SELECT hl.machinename, hl.majorversion, hl.minorversion, hcl.dependencytype
            FROM {h5p_contents_libraries} hcl
            JOIN {h5p_libraries} hl ON hcl.libraryid = hl.id
            WHERE hcl.h5pid = ?
            ORDER BY hcl.dependencytype, hl.machinename
        ", [$h5pid]);

        echo "\nContent Libraries:\n";
        foreach ($libs as $lib) {
            echo "  " . $lib->machinename . " " . $lib->majorversion . "." . $lib->minorversion . " (" . $lib->dependencytype . ")\n";
        }
    }
} else {
